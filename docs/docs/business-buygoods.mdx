...
<SectionHeader title="Response Schema" size="small" subtitle="BusinessBuyGoodsResponse — the synchronous acknowledgement returned by client.b2b.buygoods(...)" />
...
<Alert type="info" title="Implementation Options">
    ...
    - Both are available as sync and async variants (`MpesaClient`/`AsyncMpesaClient`, `BusinessBuyGoods`/`AsyncBusinessBuyGoods`) — reach for async when calling from inside an async web framework, or when sending several Buy Goods payments concurrently.
</Alert>
...
async def main():
    async with AsyncMpesaClient(
        ...
        resp = await client.b2b.buygoods(
            initiator="API_Username",
            security_credential="encrypted_credential",
            amount=239,
            party_a=123456,
...
{`
import asyncio
from mpesakit.business_buy_goods import AsyncBusinessBuyGoods, BusinessBuyGoodsRequest
...
async def main():
    async with MpesaAsyncHttpClient(env="sandbox") as http_client:
        ...
        business_buy_goods = AsyncBusinessBuyGoods(http_client=http_client, token_manager=token_manager)
        ...
        response = await business_buy_goods.buy_goods(request)
        print(response.ResponseDescription)
...
`}
        ...
        <Alert type="info" title="Direct API Details">
            - The BusinessBuyGoods service attaches the Authorization header using the provided TokenManager (sync or async, matching the client you chose).
...
async def buygoods_timeout(request: Request):
    ...
    data = BusinessBuyGoodsTimeoutCallback(**payload)  # will validate incoming fields
...
<SectionHeader title="Result Callback Schema" size="small" subtitle="BusinessBuyGoodsResultCallback — posted to ResultURL once the payment completes" />
...
<SectionHeader title="Timeout Callback Schema" size="small" subtitle="BusinessBuyGoodsTimeoutCallback — posted to QueueTimeOutURL if the payment doesn't complete in time" />

<p>Uses the same <code>Result</code> shape as the Result Callback Schema above (it's the same <code>BusinessBuyGoodsResultMetadata</code> model), just delivered to <code>QueueTimeOutURL</code> instead of <code>ResultURL</code>, typically with a non-zero <code>ResultCode</code>.</p>
...
parameters={[
    {
        ...
        dataType: "Integer/String",
        required: false,
        description: "Defaults to 0. Used by BusinessBuyGoodsResultCallbackResponse and BusinessBuyGoodsTimeoutCallbackResponse — the typed models to return from your /result and /timeout handlers respectively."
    },
    {
        name: "ResultDesc",
        type: "str",
...
<Alert type="info" title="Response Handling">
    - BusinessBuyGoodsResponse provides is_successful which normalizes success checks. Available identically on responses from the sync and async clients.
...
- Business Buy Goods:
    ...
    - Responses are returned as `BusinessBuyGoodsResponse` instances, whether awaited from the async client or returned directly from the sync client.

...
parameters={[
        ...
        },
        {
            ...
            dataType: "Integer",
            required: false,
            description: "Number of attempts (including the first try) MpesaHttpClient/MpesaAsyncHttpClient makes for a request before giving up on transient network errors (timeouts, connection errors). Defaults to 3. Configurable on MpesaClient, AsyncMpesaClient, MpesaHttpClient and MpesaAsyncHttpClient."
        },
]}
/>
...
- `Transient network errors`: MpesaHttpClient/MpesaAsyncHttpClient automatically retry timeouts and connection errors up to `max_retries` times (default 3) before raising MpesaApiException with error code REQUEST_TIMEOUT or CONNECTION_ERROR. Tune this via `max_retries` on MpesaClient/AsyncMpesaClient or the HTTP client directly. This is separate from the 401 retry pattern above, which is about expired tokens, not network failures.
